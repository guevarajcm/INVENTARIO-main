<?php
	...
	$campos="producto.producto_id,producto.producto_codigo,producto.producto_nombre,producto.sustancia_activa,producto.producto_precio,producto.caducidad,producto.prove_prod,producto.producto_stock,producto.producto_foto,producto.categoria_id,producto.usuario_id,categoria.categoria_id,categoria.categoria_nombre,usuario.usuario_id,usuario.usuario_nombre,usuario.usuario_apellido";

	if(isset($busqueda) && $busqueda!=""){
		$consulta="SELECT SQL_CALC_FOUND_ROWS $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id INNER JOIN usuario ON producto.usuario_id=usuario.usuario_id WHERE producto.producto_codigo LIKE '%$busqueda%' OR producto.producto_nombre LIKE '%$busqueda%' OR producto.sustancia_activa LIKE '%$busqueda%' OR producto.producto_precio LIKE '%$busqueda%' OR producto.caducidad LIKE '%$busqueda%' OR producto.prove_prod LIKE '%$busqueda%' OR producto.categoria_id LIKE '%$busqueda%' ORDER BY producto.producto_nombre ASC LIMIT $inicio,$registros";
			        ...
			        <div class="media-content">
			            <div class="content">
			              <p>
			                ...
			                <strong>CODIGO:</strong> '.$rows['producto_codigo'].' <b>|</b> <strong>SUSTANCIA: </strong>'.$rows['sustancia_activa'].' <b>|</b> <strong>CADUCIDAD: </strong>'.$rows['caducidad'].' <b>|</b> <strong>PROOVEDOR: </strong>'.$rows['prove_prod'].' <b>|</b> <strong>PRECIO:</strong> $'.$rows['producto_precio'].' <b>|</b> <strong>STOCK:</strong> '.$rows['producto_stock'].' <b>|</b> <strong>CATEGORIA:</strong> '.$rows['categoria_nombre'].' <b>|</b> <strong>REGISTRADO POR:</strong> '.$rows['usuario_nombre'].' '.$rows['usuario_apellido'].'
			              </p>
			            </div>
