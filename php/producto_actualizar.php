<?php
    ...
    if($sustancia!=$datos['sustancia_activa']){
    ...
    if($fechacadu!=$datos['caducidad']){
	    ...
	    if($check_fechacadu->rowCount()>0){
	        echo '
	            <div class="notification is-danger is-light">
	                <strong>¡Ocurrio un error inesperado!</strong><br>
	                LA CADUCIDAD ingresada ya se encuentra registrado, por favor elija otro
	            </div>
	        ';
	        exit();
	    }
    ...
    if($prove!=$datos['prove_prod']){
	    ...
	    if($check_prove->rowCount()>0){
	        echo '
	            <div class="notification is-danger is-light">
	                <strong>¡Ocurrio un error inesperado!</strong><br>
	                EL CAMPO PROOVEDOR ingresada ya se encuentra registrado, por favor elija otro
	            </div>
	        ';
	        exit();
	    }
    ...
    $actualizar_producto=$actualizar_producto->prepare("UPDATE producto SET producto_codigo=:codigo,producto_nombre=:nombre,sustancia_activa=:sustancia,caducidad=:caducidad,prove_prod=:proovedor,producto_precio=:precio,producto_stock=:stock,categoria_id=:categoria WHERE producto_id=:id");
